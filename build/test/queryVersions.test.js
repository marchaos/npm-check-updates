...
const queryVersions_1 = __importDefault(require("../src/lib/queryVersions"));
chai_1.default.should();
chai_1.default.use(chai_as_promised_1.default);
process.env.NCU_TESTS = 'true';
describe('queryVersions', function () {
    it('valid single package', async () => {
        const latestVersions = await (0, queryVersions_1.default)({ async: '1.5.1' }, { loglevel: 'silent' });
        return latestVersions.should.have.property('async');
    });
    it('valid packages', async () => {
        const latestVersions = await (0, queryVersions_1.default)({ async: '1.5.1', npm: '3.10.3' }, { loglevel: 'silent' });
    ...
    it('unavailable packages should be ignored', async () => {
        const result = await (0, queryVersions_1.default)({ abchdefntofknacuifnt: '1.2.3' }, { loglevel: 'silent' });
        result.should.deep.equal({});
    });
    it('local file urls should be ignored', async () => {
        const result = await (0, queryVersions_1.default)({ 'eslint-plugin-internal': 'file:devtools/eslint-rules' }, { loglevel: 'silent' });
        result.should.deep.equal({});
    });
    it('set the target explicitly to latest', async () => {
        const result = await (0, queryVersions_1.default)({ async: '1.5.1' }, { target: 'latest', loglevel: 'silent' });
        result.should.have.property('async');
    });
    it('set the target to greatest', async () => {
        const result = await (0, queryVersions_1.default)({ async: '1.5.1' }, { target: 'greatest', loglevel: 'silent' });
        result.should.have.property('async');
    });
    it('return an error for an unsupported target', () => {
        const a = (0, queryVersions_1.default)({ async: '1.5.1' }, { target: 'foo', loglevel: 'silent' });
        return a.should.be.rejected;
    });
    it('npm aliases should upgrade the installed package', async () => {
        const result = await (0, queryVersions_1.default)({
    ...
    describe('github urls', () => {
        it('github urls should upgrade the embedded version tag', async () => {
            const upgrades = await (0, queryVersions_1.default)({
        ...
        it('ignore tags that are not valid versions', async () => {
            ...
            const upgrades1 = await (0, queryVersions_1.default)({
            ...
            const upgrades2 = await (0, queryVersions_1.default)({
        ...
        it('github urls should upgrade embedded semver version ranges to prereleases with --target greatest and newest', async () => {
            const upgradesNewest = await (0, queryVersions_1.default)({
                'ncu-test-greatest-not-newest': 'https://github.com/raineorshine/ncu-test-greatest-not-newest#semver:^1.0.0',
            }, { loglevel: 'silent', target: 'newest' });
            ...
            const upgradesGreatest = await (0, queryVersions_1.default)({
                'ncu-test-greatest-not-newest': 'https://github.com/raineorshine/ncu-test-greatest-not-newest#semver:^1.0.0',
...
//# sourceMappingURL=queryVersions.test.js.map
...
const doctor = async (run, options) => {
  ...
  try {
    await npm(['run', options.testCommand || 'test'], {
  ...
  try {
    ...
    for (const [name, version] of Object.entries(upgrades)) { // eslint-disable-line fp/no-loops
      ...
      try {
        await npm(['run', options.testCommand || 'test'], { packageManager: options.packageManager }, true)
        ...
        successfulUpgrades[name] = version
      }
      catch (e) {
        failedUpgrades[name] = version
